...
# above are all of the highlight images
...
# above I am transforming and scaling the highlight images
...
# above are all of the white piece images
...
# above are all of the black piece images

def translate(value, min1, max1, min2, max2): # Translates a value from one range to another
    return min2 + (max2-min2) * ((value-min1)/(max1-min1))

def GetSprite(piece):
    sprite = None
    if piece.code == 'p': # pawn
        if piece.colour == 0:
            sprite = white_pawn
        else:
            sprite = black_pawn
    elif piece.code == 'b': # bishop
        if piece.colour == 0:
            sprite = white_bishop
        else:
            sprite = black_bishop
    elif piece.code == 'n': # knight
        if piece.colour == 0:
            sprite = white_knight
        else:
            sprite = black_knight
    elif piece.code == 'r': # rook
        if piece.colour == 0:
            sprite = white_rook
        else:
            sprite = black_rook
    elif piece.code == 'q': # queen
        if piece.colour == 0:
            sprite = white_queen
        else:
            sprite = black_queen
    else:
        if piece.colour == 0: # king
            sprite = white_king
        else:
            sprite = black_king
    transformed = pygame.transform.smoothscale(sprite, (Config.spotSize, Config.spotSize)) # Transform and scale the sprite image to the spot size
    return transformed
